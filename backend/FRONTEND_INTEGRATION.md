...
| GET | `/api/ranking-overall` | 종합·서버·직업 랭킹 (`overall_rank`, `server_rank`, `class_rank`) |
...
- **`character_exp_rate`**: 기본 정보의 경험치 비율 문자열 등.
- **`overall_rank` / `server_rank` / `class_rank`**: 종합·월드·직업 필터 기준 종합 랭킹. 넥슨 응답·첫 페이지 한계로 **null**일 수 있음. `ranking_date`는 조회 기준일.
- **`equipment.items`**: 넥슨 `item-equipment` **원본 키** (`item_equipment_slot`, `item_name`, `item_total_option`, 잠재·소울 등). 예전 `slot`/`name` 축약 필드는 제거됨.
- **`symbols.list`**: JSON 키 이름이 `list`. 각 요소는 **`symbol_name`, `symbol_icon`, `symbol_force`** 등 넥슨 원본 키.
- 일부 블록(`set_effect`, `link_skill`, 헥사 등)은 넥슨 원본 구조 그대로. 상세는 `/docs` 참고.
